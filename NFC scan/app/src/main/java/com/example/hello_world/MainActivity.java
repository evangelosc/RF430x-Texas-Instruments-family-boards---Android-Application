...
public class MainActivity extends AppCompatActivity {
    TextView Value,Byte_text,text_view,adc0,adc1,adc2,rt,temp;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        rt = (TextView) findViewById(R.id.Rt_text);
        temp = (TextView) findViewById(R.id.Temp_text);
        TEMP=0;
        RT=0;
        b_val=0;
        init_display(f_val, b_val,ADC0,ADC1,ADC2,RT,TEMP);
        ...
        if (nfc == null) {
            ...
            text_val="The phone does not support NFC!!";}
        if (!nfc.isEnabled()) {
            text_view.setText("NFC is disabled.");
            text_val="Turn on the NFC from settings!!";
        }
        ...
        mTimer = new Runnable() {
            ...
            public void run() {
                ...
                if(b_val>0)
                {
                    ...
                    init_display(f_val,b_val,ADC0,ADC1,ADC2, 0, 0);
                    RT = (float)(ADC1*100)/(ADC2); //added (KOhms)
                    if(RT>=365.0)
                    {
                        TEMP=(float)(-22.01*Math.exp(0.0001481*RT)+41.05*Math.exp(-0.001615*RT));
                    }
                    if(RT<365.0)
                    {
                        TEMP=(float)(62.82*Math.exp(-0.009056*RT)+62.79*Math.exp(-0.09722*RT));
                    }
                    init_display(f_val,b_val,ADC0,ADC1,ADC2, RT, TEMP);
                    graphLastXValue += 1d;
                    mSeries.appendData(new DataPoint(graphLastXValue, TEMP), true, 40);
                }
        ...
        for (String tech : tag.getTechList()) {
            ...
            if (tech.equals(NfcV.class.getName())) {
                ...
                systeminfo = nfcv_senseTag.transceive(cmd);
                    ...
                    text_val="Reintroduce the Phone";
                    return;
                }
                //write  to block 2
                cmd = new byte[] {
                ...
                while(b_val>0) {
                    ...
                    cmd = new byte[]{
                            ...
                            (byte) 0x07, //Reference resistor, thermistor, ADC0 sensor selected selected
                    ...
                    do {
                        try {
                            ...
                            text_val="Reintroduce the Phone"; // new version requires exception handling on each step
                            return;
                        }
    ...
    protected void init_display(String disp_value,byte disp_byte,float disp_ADC0,float disp_ADC1,float disp_ADC2,float disp_RT, float disp_TEMP)  {
        //display float
        Value.setText("Hex_Value: " + disp_value);
        //Display byte
        Byte_text.setText("Reading_Status: \n" + ((disp_byte > 0) ? "running" : "stopped"));
        //display ADC0
        adc0.setText("Light_Sensor(ADC0): "+String.valueOf(disp_ADC0)+ "V");
        adc1.setText("ADC1: "+String.valueOf(disp_ADC1)+ "V");
        adc2.setText("ADC2: "+String.valueOf(disp_ADC2)+ "V");
        rt.setText("Rt: "+String.valueOf(disp_RT)+ "KOhms");
        temp.setText("Temperature: "+String.valueOf(disp_TEMP)+ "Celsius");
